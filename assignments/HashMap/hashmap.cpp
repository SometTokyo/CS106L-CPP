...
HashMap<K, M, H> &HashMap<K, M, H>::operator = (HashMap &&map) noexcept {
    if (this != &map) {
        ...
        map._buckets_array.resize(this->bucket_count());
        //应将移后源对象置于有效的状态，也就是与默认初始化相同的状态。如果为空则会导致未定义的行为。
        //The source object, after being moved, should be set to a valid state,
        // which is equivalent to its default-initialized state. Leaving it empty may lead to undefined behavior.
        for (int i = 0; i < map.bucket_count(); ++i) {
            _buckets_array[i] = std::move(map._buckets_array[i]);
            map._buckets_array[i] = nullptr;
        }
